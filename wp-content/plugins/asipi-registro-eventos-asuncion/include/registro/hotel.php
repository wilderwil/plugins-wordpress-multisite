...
if(is_user_logged_in()){
	...
	$fecha_nac = $wpdb->get_var("SELECT date_format(value, '%Y-%m-%d') FROM pwisa_bp_xprofile_data WHERE user_id=".$user_id." AND field_id=34 " );
	...
	# Datos del evento
		...
		if($_SERVER['SERVER_NAME']=='staging.asipi.org'){ 
			#$url_form = 'https://form.jotform.com/222366398631159?id_usuario='.$user_id;
			$url_form = 'https://form.jotform.com/240285969262668?id_usuario='.$user_id;
		
		}else{
			$url_form = 'https://form.jotform.com/240493620923657?id_usuario='.$user_id;
		}
		...
		$url_form = $url_form . '&fecha_nacimiento[day]='.$d.'&fecha_nacimiento[month]='.$m.'&fecha_nacimiento[year]='.$y;   //fecha[day]=12&fecha[month]=11&fecha[year]=2020
	...
	<div id="bloque-titulo"  style="text-align: center">
		...
		<h3 style="text-align: center"><?php echo __("<!--:es-->Registro en el Hotel<!--:--><!--:en-->Hotel Registration<!--:-->") ?></h3>
	</div>
	<!--
	<div class="alerta">
	...
	<style>
		...
		<?php require_once plugin_dir_path(__FILE__) . 'jotform-hotel.php'; ?>
