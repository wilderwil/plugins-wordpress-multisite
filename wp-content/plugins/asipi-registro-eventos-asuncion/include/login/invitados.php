...
add_action('wp_ajax_nopriv_are_ajax_login_invitados','are_login_invitados');
add_action('wp_ajax_are_ajax_login_invitados','are_login_invitados');

function are_login_invitados()
{
    ...
    if ( $userType and ($_REQUEST['password']=='') ) {
        ...
        if ( $userType and ( $userType == "nonMember" ) ) {
            ...
            //echo "$returnmessage";
        }elseif ( $userType and (  $userType == "member" ) ) { 
			...
			if ( is_wp_error($user) ){
				...
				$sql_cobra_deuda = "SELECT valor FROM `evento_meta` WHERE `clave` LIKE 'cobra_deuda_antes' and evento_id='".$blog_id."'";

				$cobra_deuda = $wpdb->get_var($sql_cobra_deuda);
